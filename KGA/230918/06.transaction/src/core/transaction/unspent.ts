...
// UTXO
// 각 노드의 UTXO 데이터베이스
// 각 주소별로 가지고있는 잔액. 금액을 가지고 있는 객체의 내용이 포함되어 있다.
...
class Unspent {
  ...
  // 사용하고 나면 객체의 값을 수정하는게 아니라 한 번도 사용하지 않은 객체들이 UTXO에 담겨있고
  // 사용을 하면 제거한다.
  delete(txin: TxIn) {
  ...
  update(transaction: TransactionRow) {
    // 처리되는 트랜잭션의 내용
    // txIns : 입력값. 누가 누가에게 송금하는지 내용. 잔액 확인
  ...
  getUTXO(account: string): UnspentTxOut[] {
    // 계정 잔액의 정보를 가지고 있는 객체를 모두 조회하는 함수
  ...
  //특정 계정의 잔액 금액의 총 합을 조화하는 메서드
  getAmount(myUnspentTxOuts: UnspentTxOut[]) {
    // reduce
    // 초기 값은 0으로 두 번째 매개변수로 전달
    // 첫 번째 매개변수로 콜백함수
    //// 콜백 함수의 첫 번째 매개변수는 누적값 순회하고 변한 누적 값
    //// 콜백 함수의 두 번째 매개변수는 현재 순회하는 요소
    // 값의 합을 내보낸것.
